...
public class PatientService {
	...
	if(  patientRepository.existsByEmail(dto.getEmail())) {
	    ...
	    billingServiceGrpcClient.createBillingAccount(save.getId().toString(), save.getName(), save.getRegisterationDate().toString()); 
		return convertIntoDto;
	}
	
	public PatientResponseDTO updatePatient(UUID id, PatientRequestDTO dto) {
		 Patient byId = patientRepository.findById(id).orElseThrow(()->new PatientNotFoundException("This patient with  ID -> "+id + "  is not in our database"));
